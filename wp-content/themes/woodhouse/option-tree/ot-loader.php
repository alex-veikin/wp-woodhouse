 ...
 * Description: Theme Options UI Builder for WordPress. A simple way to create
 * & save Theme Options and Meta Boxes for free or premium themes. Version:
 * 2.6.0 Author:      Derek Herman Author URI:  http://valendesigns.com
 * License:     GPLv3 Text Domain: option-tree
 */

/**
 * Forces Plugin Mode when OptionTree is already loaded and displays an admin
 * notice.
 */
if ( class_exists( 'OT_Loader' ) && defined( 'OT_PLUGIN_MODE' )
     && OT_PLUGIN_MODE == true
) {
	...
	function ot_conflict_notice() {
		...
		echo '<div class="error"><p>'
		     . __( 'OptionTree is installed as a plugin and also embedded in your current theme. Please deactivate the plugin to load the theme dependent version of OptionTree, and remove this warning.',
				'option-tree' ) . '</p></div>';
	...
	class OT_Loader {
		...
		private function load_languages() {
			...
			define( 'OT_PLUGIN_MODE', strpos( dirname( __FILE__ ),
				...
				. basename( dirname( __FILE__ ) ) ) !== false ? true : false );
			...
			if ( OT_PLUGIN_MODE ) {
				...
				define( 'OT_LANG_DIR',
					...
					. trailingslashit( 'languages' ) );

			} else {

				if ( apply_filters( 'ot_child_theme_mode', false ) == true ) {

					$path = @explode( get_stylesheet(),
					...
					$path = @explode( get_template(),
						str_replace( '\\', '/', dirname( __FILE__ ) ) );
					$path = ltrim( end( $path ), '/' );
					define( 'OT_LANG_DIR',
						trailingslashit( trailingslashit( get_template_directory() )
						                 . $path )
						. trailingslashit( 'languages' ) . 'theme-mode' );
		...
		private function constants() {
			 ...
			 * @since     2.0.15
			 */
			define( 'OT_CHILD_THEME_MODE',
				apply_filters( 'ot_child_theme_mode', false ) );
			 ...
			 * @since     2.1
			 */
			define( 'OT_SHOW_OPTIONS_UI',
				apply_filters( 'ot_show_options_ui', true ) );
			 ...
			 * @since     2.1
			 */
			define( 'OT_SHOW_SETTINGS_IMPORT',
				apply_filters( 'ot_show_settings_import', true ) );
			 ...
			 * @since     2.1
			 */
			define( 'OT_SHOW_SETTINGS_EXPORT',
				apply_filters( 'ot_show_settings_export', true ) );
			 ...
			 * @since     2.0.10
			 */
			define( 'OT_SHOW_NEW_LAYOUT',
				apply_filters( 'ot_show_new_layout', true ) );
			 ...
			 * @since     2.1
			 */
			define( 'OT_USE_THEME_OPTIONS',
				apply_filters( 'ot_use_theme_options', true ) );
			 ...
			 * @since     2.0
			 */
			define( 'OT_ALLOW_UNFILTERED_HTML',
				apply_filters( 'ot_allow_unfiltered_html', false ) );
			 ...
			 * @since     2.4.0
			 */
			define( 'OT_POST_FORMATS',
				apply_filters( 'ot_post_formats', false ) );
			...
			if ( false == OT_THEME_MODE && false == OT_CHILD_THEME_MODE ) {
				...
				if ( true == OT_CHILD_THEME_MODE ) {
					$path_base = explode( get_template(),
						str_replace( '\\', '/', dirname( __FILE__ ) ) );
					...
					define( 'OT_DIR',
						trailingslashit( trailingslashit( get_stylesheet_directory() )
						                 . $path ) );
					define( 'OT_URL',
						trailingslashit( trailingslashit( get_stylesheet_directory_uri() )
						                 . $path ) );
				} else {
					$path_base = explode( get_template(),
						str_replace( '\\', '/', dirname( __FILE__ ) ) );
					...
					define( 'OT_DIR',
						trailingslashit( trailingslashit( get_template_directory() )
						                 . $path ) );
					define( 'OT_URL',
						trailingslashit( trailingslashit( get_template_directory_uri() )
						                 . $path ) );
				}
			}
		...
		private function hooks() {
			...
			if ( OT_META_BOXES == true ) {
				...
				add_action( 'admin_print_scripts-post-new.php',
					'ot_admin_scripts', 11 );
				add_action( 'admin_print_scripts-post.php', 'ot_admin_scripts',
					11 );

				/* add styles for metaboxes to post-new.php & post.php */
				add_action( 'admin_print_styles-post-new.php',
					'ot_admin_styles', 11 );
				add_action( 'admin_print_styles-post.php', 'ot_admin_styles',
					11 );
			...
			add_action( 'admin_bar_menu',
				'ot_register_theme_options_admin_bar_menu', 999 );
			...
			add_action( 'wp_ajax_add_the_contextual_help',
				[ $this, 'add_the_contextual_help' ] );

			/* AJAX call to create a new choice */
			add_action( 'wp_ajax_add_choice', [ $this, 'add_choice' ] );

			/* AJAX call to create a new list item setting */
			add_action( 'wp_ajax_add_list_item_setting',
				[ $this, 'add_list_item_setting' ] );
			...
			add_action( 'wp_ajax_add_social_links',
				[ $this, 'add_social_links' ] );

			/* AJAX call to retrieve Google Font data */
			add_action( 'wp_ajax_ot_google_font',
				[ $this, 'retrieve_google_font' ] );

			// Adds the temporary hacktastic shortcode
			add_filter( 'media_view_settings', [ $this, 'shortcode' ], 10, 2 );

			// AJAX update
			add_action( 'wp_ajax_gallery_update',
				[ $this, 'ajax_gallery_update' ] );
		...
		public function global_admin_css() {
			...
			$wp_38plus = version_compare( $wp_version, '3.8', '>=' ) ? true
				: false;
      ...
      ';
		...
		public function add_section() {
			echo ot_sections_view( ot_settings_id() . '[sections]',
				$_REQUEST['count'] );
			die();
		}
		...
		public function add_list_item_setting() {
			echo ot_settings_view( $_REQUEST['name'] . '[settings]',
				$_REQUEST['count'] );
			die();
		}
		...
		public function add_the_contextual_help() {
			echo ot_contextual_help_view( $_REQUEST['name'],
				$_REQUEST['count'] );
			die();
		}
		...
		public function add_list_item() {
			...
			ot_list_item_view( $_REQUEST['name'], $_REQUEST['count'], [],
				$_REQUEST['post_id'], $_REQUEST['get_option'],
				unserialize( ot_decode( $_REQUEST['settings'] ) ),
				$_REQUEST['type'] );
			die();
		}
		...
		public function add_social_links() {
			check_ajax_referer( 'option_tree', 'nonce' );
			ot_social_links_view( $_REQUEST['name'], $_REQUEST['count'], [],
				$_REQUEST['post_id'], $_REQUEST['get_option'],
				unserialize( ot_decode( $_REQUEST['settings'] ) ),
				$_REQUEST['type'] );
			die();
		...
		public function shortcode( $settings, $post ) {
			...
			if ( in_array( $pagenow, [ 'upload.php', 'customize.php' ] ) ) {
				return $settings;
			}
			...
			if ( ! is_object( $post ) ) {
				...
				if ( $post_id == 0
				     && function_exists( 'ot_get_media_post_ID' )
				) {
			...
			$settings['ot_gallery']
				= [ 'shortcode' => "[gallery id='{$settings['post']['id']}']" ];
		...
		public function ajax_gallery_update() {
			...
			if ( ! empty( $_POST['ids'] ) ) {
				...
				foreach ( $_POST['ids'] as $id ) {
					...
					$return .= '<li><img  src="' . $thumbnail[0]
					           . '" width="75" height="75" /></li>';
		...
		public function retrieve_google_font() {
			...
			if ( isset( $_POST['field_id'], $_POST['family'] ) ) {
				...
				echo json_encode( [
					'variants' => ot_recognized_google_font_variants( $_POST['field_id'],
						$_POST['family'] ),
					'subsets'  => ot_recognized_google_font_subsets( $_POST['field_id'],
						$_POST['family'] ),
				] );
				exit();
		...
		public function change_image_button( $translation, $text, $domain ) {
			...
			if ( $pagenow == apply_filters( 'ot_theme_options_parent_slug',
				...
				return apply_filters( 'ot_upload_text',
